...
const list = (value: unknown): User[] =>
  Array.isArray(value)
    ? (value as User[])
    : (((value as { items?: User[] })?.items || []) as User[]);
...
export default function AdminUsersPage() {
  ...
  const load = async () => {
    const [result, typeResult] = await Promise.all([
      userApi.getUsersPagination(page, 8),
    ...
    setTypes(
      (Array.isArray(typeResult.data) ? typeResult.data : []) as {
        maLoaiNguoiDung: string;
  ...
  const visible = users.filter((user) =>
    `${user.taiKhoan} ${user.hoTen} ${user.email}`
      .toLowerCase()
  ...
  const submit = async (event: FormEvent) => {
    ...
    const payload = form.matKhau
      ? form
      : (({ matKhau: _password, ...withoutPassword }) => withoutPassword)(form);
    if (editing) await userApi.updateUser(payload);
    else await userApi.addUser(payload);
    setOpen(false);
    await load();
  };
  const remove = async (user: User) => {
    if (window.confirm(`Xóa tài khoản "${user.taiKhoan}"?`)) {
      await userApi.deleteUser(user.taiKhoan);
      await load();
    }
  };
  return (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {visible.map((user) => (
                <tr key={user.taiKhoan} className="hover:bg-[#fbfdfc]">
                  <td className="px-5 py-4 font-semibold text-[#123b3a]">
                    {user.taiKhoan}
                  </td>
                  <td className="px-5 py-4 text-slate-700">{user.hoTen}</td>
                  <td className="px-5 py-4 text-slate-500">{user.email}</td>
                  <td className="px-5 py-4">
                    <span className="rounded-full bg-[#e6f4f1] px-2.5 py-1 text-xs font-bold text-[#237c73]">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-semibold text-slate-600">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, maLoaiNguoiDung: e.target.value })
                  }
                  className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 outline-none"
                >
                  {types.length ? (
                    types.map((type) => (
                      <option
