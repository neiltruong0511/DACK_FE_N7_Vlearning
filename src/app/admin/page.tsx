    ...
    <div>
      <header className="mb-9 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-[#123b3a] sm:text-4xl">
            Chào mừng trở lại
          </h1>
          <p className="mt-2 text-sm text-slate-500">
            Theo dõi và vận hành nội dung học tập của VLearning.
          </p>
        </div>
        <span className="rounded-full border border-[#d7e4e2] bg-white px-4 py-2 text-xs font-semibold text-slate-500">
          Quản trị viên
        </span>
      </header>
      ...
      <section className="grid gap-5 md:grid-cols-3">
        {cards.map(({ label, value, href, icon: Icon, tone }) => (
          <Link
            key={href}
            href={href}
            className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_8px_30px_rgba(30,65,64,0.05)] transition hover:-translate-y-0.5 hover:shadow-lg"
          >
            <div className="flex items-start justify-between">
              ...
              <ArrowUpRight className="h-5 w-5 text-slate-300 transition group-hover:text-[#237c73]" />
            </div>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(30,65,64,0.05)]">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <Link
              href="/admin/courses"
              className="rounded-xl border border-slate-200 p-4 text-sm font-bold text-slate-700 transition hover:border-[#8bc9c0] hover:bg-[#f3fbf9]"
            >
              Quản lý khóa học{" "}
              <span className="float-right text-[#237c73]">→</span>
            </Link>
            <Link
              href="/admin/categories"
              className="rounded-xl border border-slate-200 p-4 text-sm font-bold text-slate-700 transition hover:border-[#8bc9c0] hover:bg-[#f3fbf9]"
        ...
        <div className="rounded-2xl bg-[#123b3a] p-6 text-white shadow-[0_8px_30px_rgba(18,59,58,0.18)]">
          ...
          <h2 className="mt-4 text-2xl font-black leading-tight">
            Nội dung tốt tạo nên hành trình học tốt.
          </h2>
          <p className="mt-3 text-sm leading-6 text-teal-50/70">
            Quản lý khóa học, danh mục và tài khoản trong một không gian gọn
            gàng.
          </p>
        </div>
      </section>
