...
// Thêm Interface định nghĩa kiểu dữ liệu cho Khóa học để fix lỗi "any"
interface CourseData {
  maKhoaHoc: string;
  tenKhoaHoc: string;
}

export default function UserEnrollmentModal({ isOpen, onClose, taiKhoan }: UserEnrollmentModalProps) {
  const [activeTab, setActiveTab] = useState<"unregistered" | "pending" | "approved">("unregistered");
  
  const { unregistered, pending, approved } = useCoursesByUser(taiKhoan || "");

  const isLoading = unregistered.isLoading || pending.isLoading || approved.isLoading;
  ...
  const currentData = activeTab === "unregistered" ? unregistered.data?.data 
                    : activeTab === "pending" ? pending.data?.data 
                    : approved.data?.data;

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Quản lý khóa học - Học viên: ${taiKhoan}`} maxWidth="max-w-5xl">
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.label}
              <span className={`ml-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${activeTab === tab.id ? "bg-[#e4c77b] text-[#123b3a]" : "bg-slate-300 text-slate-700"}`}>
                {tab.count}
              </span>
            </button>
          );
      ...
      <div className="min-h-[300px]">
        {isLoading ? (
          <div className="flex h-[300px] items-center justify-center text-slate-500">Đang tải dữ liệu...</div>
        ) : currentData && currentData.length > 0 ? (
          <ul className="space-y-3">
            {/* Đã sửa course: any thành course: CourseData */}
            {currentData.map((course: CourseData, idx: number) => (
              <li key={idx} className="flex items-center justify-between rounded-xl border border-slate-100 bg-[#f5f8f8] p-4 shadow-sm transition hover:shadow-md">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-teal-100 text-[#123b3a]">
                    <BookOpen className="h-6 w-6" />
                  </div>
                  <div>
                    <h4 className="font-bold text-[#123b3a]">{course.tenKhoaHoc}</h4>
                    <span className="text-sm text-slate-500">Mã KH: {course.maKhoaHoc}</span>
                  </div>
                </div>
